...
LICENSE="GPL-2+"
KEYWORDS="~amd64 ~x86"
IUSE="flashexport mediawiki +remotestorage vkontakte +reverseimagesearch"
...
COMMON_DEPEND="
	...
	reverseimagesearch? ( dev-vcs/git )
"
...
src_prepare() {
	if use reverseimagesearch ; then
		git clone --bare --depth 2 https://github.com/YoukaiCat/kipi-plugins.git
		export GIT_DIR=kipi-plugins.git/
		export GIT_WORK_TREE=.
		git format-patch HEAD^ --stdout | git apply -p1
	fi
