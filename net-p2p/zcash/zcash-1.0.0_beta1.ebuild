...
SRC_URI="https://github.com/zcash/zcash/archive/v1.0.0-beta1.tar.gz -> ${PV}.tar.gz"
...
S=${WORKDIR}/zcash-1.0.0-beta1

src_compile() {
	sed -i 's/\.\/b2/\.\/b2 --ignore-site-config/g' depends/packages/boost.mk #boost build error
	unset ABI #libgmp: onfigure: error: ABI=amd64 is not among the following valid choices: 64 x32 32
...
src_install() {
	...
	ewarn "$ wget -O - https://raw.githubusercontent.com/zcash/zcash/v1.0.0-beta1/zcutil/fetch-params.sh | bash"
	ewarn " "
